<#--  This file uses
  portalCurrentURL
  change_menu_url
  change_menu_url_boolean
  defined in init_custom.ftl file
-->

<#--  Menu Change Route URL -->
<#assign change_menu_url = themeDisplay.getThemeSetting("change-menu-url")?trim>

<#assign portalCurrentURL = themeDisplay.getPortalURL() + themeDisplay.getURLCurrent()>

<#if change_menu_url != "" && change_menu_url != "/">
		...
		<ul class="${project_prefix}-menu__main-list">
			<#list nav_items as navItem>
              ...
              <a href="#" onclick="return false;" class="${project_prefix}-menu__link">${navItem.getName()}</a>
                      ...
                      <#else>
                        <#assign routeURLChild = portalCurrentURL + childItem.getLayout().getFriendlyURL(locale)?replace("/", "${change_menu_url}")>
                        <#assign routeURLChild = routeURLChild?replace("/" + change_menu_url, change_menu_url)>
                        ...
                        <ul class="${project_prefix}-menu__submenu-children-list">
												  <#list childItem.getChildren() as grandChild>
														<li>
                              <#assign routeURLgrandChild = portalCurrentURL + grandChild.getLayout().getFriendlyURL(locale)?replace("/", "${change_menu_url}")>
                              <#assign routeURLgrandChild = routeURLgrandChild?replace("/" + change_menu_url, change_menu_url)>
            ...
            <#else>
              <#assign routeURLnavItem = portalCurrentURL + navItem.getLayout().getFriendlyURL(locale)?replace("/", "${change_menu_url}")>
              <#assign routeURLnavItem = routeURLnavItem?replace("/" + change_menu_url, change_menu_url)>
